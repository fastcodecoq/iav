...
if ($page == 0) $page = 1;					
	...
	*/
	...
	{	
		...
		//previous button
		if ($page > 1) 
			$paginacion.= "<a href=\"$pagina?page=$prev\">« Anterior</a>";
		else
		...
		{	
			for ($contador = 1; $contador <= $total_paginas; $contador++)
			{
				if ($contador == $page)
					...
					$paginacion.= "<a href=\"$pagina?page=$contador\">$contador</a>";					
			}
		}
		elseif($total_paginas > 5 + ($adjacents * 2))	//enough pages to hide some
		{
			...
			if($page < 1 + ($adjacents * 2))		
			{
				...
				$paginacion.= "<a href=\"$pagina?page=$total_paginas\">$total_paginas</a>";		
			}
			//in middle; hide some front and some back
			elseif($total_paginas - ($adjacents * 2) > $page && $page > ($adjacents * 2))
			{
				$paginacion.= "<a href=\"$pagina?page=1\">1</a>";
				$paginacion.= "<a href=\"$pagina?page=2\">2</a>";
				$paginacion.= "...";
		...
		if ($page < $total_paginas) 
			$paginacion.= "<a href=\"$pagina?page=$next\">Siguiente »</a>";
		else
