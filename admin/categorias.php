...
$eliminados = $no_eliminados = 0;
	...
	{
		$consulta = "SELECT * FROM categorias WHERE id = ".$_POST["chk_categoria"][$i];
		$resultado_del = mysql_query($consulta, $conexion);	
		$registro_del = mysql_fetch_array($resultado_del);
		
		$eliminacion = "DELETE FROM categorias WHERE id = ".$_POST["chk_categoria"][$i];
      ...
      <table width="98%" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr>
          <td class="box" valign="top"><div class="content">
            ...
            <?php
			$consulta = "SELECT * FROM categorias ORDER BY id ";
			$resultado = mysql_query($consulta, $conexion);
			$num_registros = mysql_num_rows($resultado);
			$total_paginas = ceil($num_registros / $TAMANO_PAGINA); 
			$registro = mysql_fetch_array($resultado);
			
			$consulta = "SELECT * FROM categorias ORDER BY nombre LIMIT $inicio , $TAMANO_PAGINA  ";
			$resultado = mysql_query($consulta, $conexion);
			...
			?>
              ...
              <?php
				...
				?>
              <tr>
                <td style="text-align: center;"><input name="chk_categoria[]" type="checkbox" value="<?php echo $registro["id"]; ?>" /></td>
